...
function Results({ result, crops, area, form }) {
  ...
  return <div className="opt-results">
    ...
    <section className="provenance"><div><span className="kicker">Trazabilidad del cálculo</span><h3>{result.provenance?.snapshot_id || "Catálogo económico"}</h3><p>{result.provenance?.method || "Supuestos editables para planeación; no son una cotización en tiempo real."}</p></div><div className="source-links">{(result.provenance?.sources || []).map((s) => <a href={s.url} target="_blank" rel="noreferrer" key={s.id}>{s.id} ↗</a>)}</div></section>    <div className="result-actions"><button className="button" onClick={() => downloadCommercialPdf(result, crops, form)}>Descargar PDF</button><button className="button secondary" onClick={() => downloadCommercialCsv(result, crops, form)}>Descargar CSV</button></div>    
